...
def clean_text(text):
    ...
    text = re.sub(r'\d+', '', text) # 数字の置換。意味的な分析ででは、数字の具体的な値を使えないことが多く、いたずらにボキャブラリを増やすだけで、後のタスクでは役に立たないことが多いため
    text = re.sub(r'[!-/:-@[-`{-~]', r' ', text) # 半角記号の置換
    text = re.sub(u'[■-♯【】]', '', text) # 全角記号の置換 (ここでは0x25A0 - 0x266Fのブロックのみを除去)
    text = text.replace(u' ', '').replace(u'　', '').replace(u'「', '').replace(u'」', '').replace('・', "").replace('、', "").replace('年', "").replace('月', "").replace('日', "")
    return text